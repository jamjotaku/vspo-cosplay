...
const memberIcons = { "花芽すみれ": "👾💤", "花芽なずな": "Sushi", "小雀とと": "🔫🐥", "一ノ瀬うるは": "🌠", "胡桃のあ": "🧸♔", "橘ひなの": "🍫💘", "如月れん": "⏰", "英リサ": "💐", "空澄セナ": "🗝♠︎", "兎咲ミミ": "🐰🍭", "神成きゅぴ": "🌩", "八雲べに": "💄💚", "藍沢エマ": "🥞💫", "紫宮るな": "☪🐾", "猫汰つな": "🍒✨", "白波らむね": "🐻‍❄️🏖", "小森めと": "🪐", "夢野あかり": "🍼", "夜乃くろむ": "💀⛓", "紡木こかげ": "📘💧", "千燈ゆうひ": "🫠", "蝶屋はなび": "🦋🎆", "甘結もか": "🕹🔖", "銀城サイネ": "🎈", "龍巻ちせ": "🐉🌪" };
...
export default function Gallery() {
  ...
  const [themeColor, setThemeColor] = useState('#00f2ff'); // カラー同期用
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (session) {
        const { data: prof } = await supabase.from('profiles').select('theme_color').eq('id', session.user.id).single();
        if (prof?.theme_color) {
          setThemeColor(prof.theme_color);
          document.documentElement.style.setProperty('--v-accent', prof.theme_color);
        }
      }
  ...
  return (
    <div className="g-root">
      ...
      {activeStory && (
        <div className="sv-root" onClick={()=>setActiveStory(null)}>
          <div className="sv-prog">
            {stories[activeStory.memberIndex].images.map((_,i)=>(
              <div key={i} className="sv-seg">
                <div className="sv-fill" style={{width:i < activeStory.slideIndex ? '100%' : (i === activeStory.slideIndex ? '100%' : '0%'), transition:i===activeStory.slideIndex?'4s linear':'none', background: themeColor}}></div>
              </div>
            ))}
          </div>
          <div className="sv-stage" onClick={e=>e.stopPropagation()}>
            ...
            <div className="sv-label-box" style={{border: `1px solid ${themeColor}`}}>{stories[activeStory.memberIndex].member}</div>
          </div>
      ...
      <style jsx global>{`
        ...
        .g-search-wrap input:focus { border-color: var(--v-accent); width: 220px; }
        ...
        .s-node:hover .s-ring { border-color: var(--v-accent); box-shadow: 0 0 15px var(--v-accent); }
        .s-node label { display:block; font-size:10px; margin-top:10px; color:#555; font-weight: 700; }
        ...
        .g-chips button.active { color: var(--v-accent); }
        ...
        .g-name { font-weight:400; font-size: 16px; color: #eee; }
        ...
        .sv-label-box { position:absolute; top:30px; left:30px; background:rgba(255,255,255,0.1); backdrop-filter: blur(10px); padding:8px 20px; border-radius:50px; font-size:14px; font-weight: 800; }
