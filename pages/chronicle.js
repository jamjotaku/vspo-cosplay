...
export default function ChroniclePage() {
      ...
      complete: (results) => {
        ...
        raw.forEach((d, i) => {
          ...
          [mName, cName].forEach((name, idx) => {
            ...
            nodeMap.get(name).degree += 1; // 登場回数をカウント
          });
  ...
  useEffect(() => {
    ...
    node.append("circle")
      ...
      .attr("filter", d => d.degree > 10 ? "drop-shadow(0 0 12px rgba(0,242,255,0.6))" : "none");

    // ラベル（大きいノードだけ常に表示、小さいのはホバー時のみ、などの調整も可能）
    node.append("text")
      .text(d => d.id)
      .attr("x", d => radiusScale(d.degree) + 5)
      .attr("y", 4)
      .attr("fill", "#fff")
      .attr("font-size", d => Math.max(10, radiusScale(d.degree) / 1.5) + "px")
